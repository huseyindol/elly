-- ============================================================
-- Chat System Migration — tenant DBs
-- Tenant DB'lerine (tenant1, tenant2, ...) chat tablolarını kurar.
-- basedb'de bu tablolar db-migration-chat.sql + guest-chat-migration.sql
-- ile zaten oluşturulmuştur; bu script tenant'lara uygulayın.
--
-- Kullanım:
--   psql elly_tenant1 -f db-migration-chat-tenant.sql
--   psql elly_tenant2 -f db-migration-chat-tenant.sql
-- ============================================================

-- 1. Ana tablolar (basedb ile aynı şema)
...
CREATE TABLE IF NOT EXISTS chat_messages (
  id           UUID    PRIMARY KEY DEFAULT gen_random_uuid(),
  group_id     UUID    NOT NULL REFERENCES chat_groups(id) ON DELETE CASCADE,
  sender_id    BIGINT  REFERENCES users(id),          -- nullable: guest mesajları için
...
-- 2. Guest chat kolonları (guest-chat-migration.sql eşdeğeri)
